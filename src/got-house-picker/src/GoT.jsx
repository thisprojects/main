...
class Houselist extends React.Component {
  render() {
    ...
    if (!results.name && click === true) {
      // if we have NOT had the results back from the GoT API BUT the user has clicked to submit their name

      return <h1>...Loading</h1>;
    } else if (results.name && click === true) {
      // when the results from the API are in

      return (
        // return API info in list form - titles resturns results from 2nd api call - which is to the characters API
        <div>
          <div className="nameBox">
            <h1>
              {form.charAt(0).toUpperCase() + form.slice(1)} of {results.name}
            </h1>
          </div>
          <div className="resultsTable">
            <table>
              <tbody>
                ...
                {members.name ? (
                  <tr>
                    <td>Current Lord: </td> <td>{members.name}</td>
                  </tr>
                ) : (
                  <tr>
                    <td>Current Lord:</td> <td>None</td>
                  </tr>
                )}
                ...
                <tr>
                  <td>Founded: </td>
                  <td>
                    {results.founded ? results.founded : "No Information"}
                  </td>
                </tr>
                <tr>
                  <td>Died Out: </td>
      ...
      return (
        // if our submit button has not yet been clicked - return an empty div
  ...
  _arrayMap(test) {
    ...
    if (test != "") {
      // if the array argument is not empty
      ...
      return (
        <div key={input.toString()}>
          {input.map((
            input // map the array and return the output items in HTML list
          ) => (
      ...
      return (
        // return a list containing the string "none"
...
class app extends React.Component {
  ...
  render() {
    return (
      // displays our name form / box and then calls our house list componenet - to display results from API
      <div>
  ...
  _fetchIT(event) {
    ...
    fetch(
      ...
      .then(() => {
        // the first api call fetches random house information

        if (this.state.results.swornMembers.length) {
          // if a character is given as current house lord  - we then contact characters API and request info on character

          console.log("Characters api results");

          console.log(this.state.results.swornMembers);

          fetch(this.state.results.swornMembers[0])
            .then(response => response.json())
  ...
  _randomHouse() {
    // pick random house and reset our state contents
